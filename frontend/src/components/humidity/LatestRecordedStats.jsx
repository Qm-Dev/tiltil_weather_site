...
export default function LatestRecordedStats({ latestRecord, latestMaxMin, last24Hours }) {
    ...
    // Dates
    const parsedDateLatestRecord = latestRecord.timestamp ? parseISO(latestRecord.timestamp).toLocaleDateString("es-CL", { hour: "2-digit", minute: "2-digit" , hour12: false }) : null;
    const parsedDateMaxHumidity = latestMaxMin.max_timestamp ? parseISO(latestMaxMin.max_timestamp).toLocaleDateString("es-CL", { hour: "2-digit", minute: "2-digit" , hour12: false }) : null;
    const parsedDateMinHumidity = latestMaxMin.min_timestamp ? parseISO(latestMaxMin.min_timestamp).toLocaleDateString("es-CL", { hour: "2-digit", minute: "2-digit" , hour12: false }) : null;
    ...
    return (
        ...
        <div className="row text-center justify-content-center mx-auto gap-3 w-75 mt-3">
            ...
            <h3 className="fw-bold text-black">Today's Humidity Range: {(latestMaxMin?.max_hum && latestMaxMin?.min_hum) ? `${latestMaxMin.max_hum - latestMaxMin.min_hum}%` : "N/A"}</h3>
        </div>
        <div className="row text-center justify-content-center mx-auto gap-3 w-75 mt-3">
